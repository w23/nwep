...
const char *raymarch_glsl =
 ...
 "}"
 "vec3 e(vec3 v,vec3 a,vec3 m,vec3 x,float z,float y)"
 "{"
   "vec3 i=vec3(0.);"
   "for(int h=0;h<LN;++h)"
     "{"
       "vec3 s=c[h]-v;"
       "float w=dot(s,s),t=sqrt(w);"
       "s=normalize(s);"
       "vec3 n=d(v+.02*s,s,t);"
       "if(n.x+.2<t)"
         "continue;"
       "vec3 o=normalize(a+s),g=mix(vec3(.04),x,z);"
       "float r=max(dot(o,a),0.),p=max(dot(m,a),0.),u=max(dot(m,s),0.),b=max(dot(m,o),0.);"
       "vec3 L=g+(1.-g)*pow(1.-r,5.);"
       "float N=e(p,y)*e(u,y);"
       "vec3 F=d(b,y)*N*L/(4.*p*u+.001);"
       "i+=((vec3(1.)-L)*(1.-z)*x/f+F)*u*l[h]/w;"
     "}"
   "return i;"
 "}"
 ...
 "vec3 g(vec3 v)"
 "{"
   "return vec3(v.x*cos(v.z),v.y,v.x*sin(v.z));"
 "}"
 "void main()"
 "{"
   ...
   "vec3 x=g(C)+.1*r(v*3.);"
   "mat3 i=h(x,g(A),m.xzx);"
   "vec3 z=i*normalize(vec3(f,-1.44)),s=m.xxx;"
   "const float y=40.;"
   "vec3 o=d(x,z,y),n=x+o.x*z,p,u,N;"
   "float w,b;"
   "d(n,N,u,p,b,w);"
   "s=u+e(n,-z,N,p,w,b);"
   "gl_FragColor=vec4(s,o.x);"
 "}";
