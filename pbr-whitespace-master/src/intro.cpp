...
void initShader(short* pid, const char* fs)
{
	...
	const int fsId2 = oglCreateShader(GL_FRAGMENT_SHADER);
	const int fsId3 = oglCreateShader(GL_FRAGMENT_SHADER);
...
{
	EXT_Init();
	initShader(&pid, fragment_glsl);
	initShader(&pid, out_glsl);
}
#endif
