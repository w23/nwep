...
"}"
"vec3 a(vec3 v,vec3 f,vec3 i,vec3 r,float n,float b)"
"{"
"vec3 P=vec3(0.);"
"for(int g=0;g<LN;++g)"
"{"
...
"if(p.r+.2<x)"
"continue;"
"vec3 e=normalize(f+d),l=mix(vec3(.04),r,n);"
"float t=max(dot(e,f),0.),u=max(dot(i,f),0.),C=max(dot(i,d),0.),I=max(dot(i,e),0.);"
"vec3 L=l+(1.-l)*pow(1.-t,5.);"
"float z=a(u,b)*a(C,b);"
"vec3 h=K(I,b)*z*L/(4.*u*C+.001);"
"P+=((vec3(1.)-L)*(1.-n)*r/m+h)*C*o[g]/s;"
"}"
"return P;"
"}"
...
"vec3 f=normalize(vec3(m,-1.44)),i,P=vec3(0.);"
"if(T<23.)"
...
"vec3 e=vec3(0.);"
"const float g=40.;"
"vec3 s=K(i,f,g),x=i+s.r*f,l,C,N;"
"float p,u;"
"K(x,N,C,l,u,p);"
"e=C+a(x,-f,N,l,p,u);"
"gl_FragColor=vec4(e,s.r);"
"}";
