...
$SG47338 DB	'uniform int Q;uniform vec2 V;float T=Q*.001;const vec3 v'
	...
	DB	'rn length(v)-m;}vec3 s(vec3 v,vec3 m){return mod(v,m)-m*.5;}f'
	...
	DB	';}vec3 a(vec3 v,vec3 f,vec3 i,vec3 r,float n,float b){vec3 P='
	DB	'vec3(0.);for(int g=0;g<LN;++g){vec3 d=c[g]-v;float s=dot(d,d)'
	DB	',x=sqrt(s);d=normalize(d);vec3 p=K(v+.02*d,d,x);if(p.r+.2<x)c'
	DB	'ontinue;vec3 e=normalize(f+d),l=mix(vec3(.04),r,n);float t=ma'
	DB	'x(dot(e,f),0.),u=max(dot(i,f),0.),C=max(dot(i,d),0.),I=max(do'
	DB	't(i,e),0.);vec3 L=l+(1.-l)*pow(1.-t,5.);float z=a(u,b)*a(C,b)'
	DB	';vec3 h=K(I,b)*z*L/(4.*u*C+.001);P+=((vec3(1.)-L)*(1.-n)*r/m+'
	DB	'h)*C*o[g]/s;}return P;}mat3 a(vec3 v,vec3 m,vec3 f){vec3 r=no'
	DB	'rmalize(v-m),i=normalize(cross(f,r));f=cross(r,i);return mat3'
	DB	'(i,f,r);}void main(){vec2 m=gl_FragCoord.rg/V*2.-1.;m.r*=V.r/'
	DB	'V.g;float r=11.;c[0]=vec3(r,6.,r);c[1]=vec3(r,6.,-r);c[2]=vec'
	DB	'3(-r,6.,-r);c[3]=vec3(-r,6.,r);c[4]=vec3(0.);o[0]=30.*vec3(.7'
	DB	',.35,.45)*mix(1.,n(T*20.),.3);o[1]=30.*vec3(.7,.35,.15)*mix(1'
	DB	'.,n(T*20.+10.),.3);o[2]=30.*vec3(.3,.35,.75)*mix(1.,n(T*20.+2'
	DB	'0.),.3);o[3]=30.*vec3(.7,.35,.15)*mix(1.,n(T*20.+30.),.3);o[4'
	DB	']=smoothstep(44.,50.,T)*50.*vec3(1.)*mix(1.,n(T*20.+30.),.3);'
	DB	'vec3 f=normalize(vec3(m,-1.44)),i,P=vec3(0.);if(T<23.)i=vec3('
	DB	'mix(40.,11.,T/23.),2.,0.);else if(T<34.){float d=T-34.;i=vec3'
	DB	'(cos(d*.1)*13.,2.,sin(d*.1)*14.);P=vec3(20.,0.,20.);}else if('
	DB	'T<62.)i=vec3(cos(T*.1)*13.,2.,sin(T*.1)*14.);else{float b=T*2'
	DB	'./10.48,d=floor(b);i=13.*mix(vec3(n(d),n(d+4.),n(d+5.)),vec3('
	DB	'n(d+17.),n(d+41.),n(d+35.)),b-d);}float d=T*3.;i+=.1*vec3(n(d'
	DB	'),n(d+1.),n(d+3.));mat3 b=a(i,P,v.rbr);i+=b*vec3(m*.01,0.);f='
	DB	'b*f;vec3 e=vec3(0.);const float g=40.;vec3 s=K(i,f,g),x=i+s.r'
